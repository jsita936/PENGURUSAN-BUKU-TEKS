        ...
        <div className="max-w-4xl w-full text-center space-y-12 animate-in fade-in duration-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto w-full">
            <button onClick={() => adminSettings.isRegistered ? setAuthView('admin_auth') : setAuthView('setup')} className="bg-white/10 border-2 border-white/10 p-10 rounded-[3rem] text-left hover:bg-white/20 transition group border-indigo-400/30">
              ...
              <p className="text-indigo-200 text-sm">Kawalan stok, urus ahli & peranti.</p>
            ...
            <button onClick={() => setAuthView('guru_auth')} className="bg-white/10 border-2 border-white/10 p-10 rounded-[3rem] text-left hover:bg-white/20 transition group border-emerald-400/30">
              ...
              <p className="text-indigo-200 text-sm">Pinjaman layan diri & profil saya.</p>
  ...
  if (authView === 'setup' || authView === 'admin_auth' || authView === 'guru_auth') {
    ...
    return (
      <div className="min-h-screen bg-indigo-950 flex items-center justify-center p-6">
        <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl w-full max-w-md text-center animate-in zoom-in border-4 border-indigo-100">
          ...
          <form onSubmit={isSetup ? handleRegisterAdmin : isGuru ? (e) => { e.preventDefault(); handleSetUser(); } : handleAdminLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full py-6 bg-indigo-600 text-white font-black rounded-[2rem] hover:bg-indigo-700 transition uppercase tracking-widest text-xs shadow-xl active:scale-95 border-b-4 border-indigo-800">Masuk Sistem</button>
            <button type="button" onClick={() => setAuthView('landing')} className="w-full py-4 text-slate-400 text-[10px] font-black uppercase tracking-widest hover:text-indigo-600 transition">Kembali</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 text-indigo-950 font-['Plus_Jakarta_Sans']">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 pb-24 bg-slate-50">
          ...
          {activeTab === 'inventory' && (
            <div className="space-y-8 animate-in fade-in">
              ...
              {/* TABS TAHUN 1-6 UNTUK SEMUA PAPARAN INVENTORI */}
              <div className="flex bg-white p-1 rounded-[1.5rem] shadow-sm border border-slate-200 overflow-x-auto no-scrollbar">
                {YEARS.map(y => (
                  <button 
                    key={y} 
                    onClick={() => setSelectedYear(y)} 
                    className={`flex-1 min-w-[100px] py-4 rounded-xl font-black transition-all uppercase text-[11px] tracking-widest ${selectedYear === y ? 'bg-indigo-600 text-white shadow-lg translate-y-[-1px]' : 'text-slate-500 hover:bg-slate-50 hover:text-indigo-600'}`}
                  >
                    Tahun {y}
                  </button>
                ))}
              ...
              {inventoryView === 'Guru' ? (
                <div className="space-y-12">
                   {/* PENGASINGAN MENGIKUT JENIS BUKU (TEKS VS AKTIVITI) DALAM TAHUN PILIHAN */}
                   <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                      {['Buku Teks', 'Buku Aktiviti'].map(type => {
                        const filteredBooks = books.filter(b => b.year === selectedYear && b.type === type);
                        if (filteredBooks.length === 0) return null;

                        return (
                          <div key={type} className="space-y-6">
                            <div className="flex items-center gap-3 bg-indigo-50/50 p-4 rounded-2xl border-2 border-indigo-100">
                              <div className={`w-3 h-8 rounded-full ${type === 'Buku Teks' ? 'bg-emerald-500' : 'bg-orange-500'}`}></div>
                              <h4 className="text-md font-black uppercase text-indigo-900 tracking-tighter">{type.toUpperCase()} - TAHUN {selectedYear}</h4>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              {filteredBooks.map(book => {
                                const hasBorrowed = getActiveLoans(userName).some(l => l.bookId === book.id);
                                return (
                                  <div key={book.id} className="bg-white border-2 border-slate-100 rounded-[2rem] p-6 shadow-sm hover:shadow-xl transition-all group hover:border-indigo-400 relative overflow-hidden">
                                     <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none group-hover:scale-125 transition-transform"><BookOpen size={48}/></div>
                                     <h4 className="font-black text-indigo-950 text-[11px] mb-2 uppercase h-10 overflow-hidden leading-tight group-hover:text-indigo-600 transition-colors">{book.title}</h4>
                                     <p className="text-[9px] text-slate-500 font-black uppercase mb-4 italic">Stok: <span className={book.stock < 20 ? 'text-rose-600 font-bold' : 'text-indigo-600'}>{book.stock}</span></p>
                                     <div className="grid grid-cols-2 gap-2 relative z-10">
                                       <button onClick={() => handleAction(book.id, 'Pinjaman', userName, 'Guru')} className="py-3 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase shadow-md active:scale-95 transition-all hover:bg-indigo-700 border-b-2 border-indigo-800">Pinjam</button>
                                       <button disabled={!hasBorrowed} onClick={() => handleAction(book.id, 'Pemulangan', userName, 'Guru')} className={`py-3 rounded-xl text-[9px] font-black uppercase transition-all ${hasBorrowed ? 'bg-emerald-600 text-white shadow-md active:scale-95 hover:bg-emerald-700 border-b-2 border-emerald-800' : 'bg-slate-50 text-slate-300'}`}>Pulang</button>
                                     </div>
                                  </div>
                                );
                              })}
                ...
                /* Paparan Inventori Murid (Berdasarkan Tahun terpilih) */
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                   {members.filter(m => m.type === 'Murid' && m.year === selectedYear).map(student => {
                     ...
                     return (
                       <div key={student.id} className="bg-white rounded-[2.5rem] border-2 border-slate-100 shadow-sm overflow-hidden transition-all hover:border-indigo-400 group">
                         <button onClick={() => { const newExp = new Set(expandedStudents); isExpanded ? newExp.delete(student.id) : newExp.add(student.id); setExpandedStudents(newExp); }} className={`w-full px-6 py-5 flex justify-between items-center transition-colors ${isExpanded ? 'bg-indigo-50/50' : 'hover:bg-slate-50'}`}>
                            <div className="flex items-center gap-3 text-left">
                               <div className="w-10 h-10 rounded-xl bg-indigo-600 text-white flex items-center justify-center font-black shadow-inner group-hover:rotate-6 transition-transform">{student.name.charAt(0)}</div>
                               <div><h4 className="font-black text-indigo-950 uppercase text-[10px] truncate w-32">{student.name}</h4><p className="text-[8px] font-bold text-slate-500 uppercase">{activeLoans.length} Pinjaman Aktif</p></div>
                            </div>
                            {isExpanded ? <ChevronDown size={18} className="text-indigo-600" /> : <ChevronRight size={18} className="text-slate-300" />}
                         </button>
                         {isExpanded && (
                           <div className="p-6 space-y-3 bg-white border-t-2 border-slate-50 animate-in slide-in-from-top-2">
                              <button onClick={() => { setBorrowingStudent(student); setIsBorrowModalOpen(true); }} className="w-full py-4 bg-indigo-600 text-white rounded-xl font-black uppercase text-[9px] flex items-center justify-center gap-2 mb-2 hover:bg-indigo-700 transition shadow-md border-b-2 border-indigo-800"><Plus size={14}/> Daftar Pinjaman Baru</button>
                              {activeLoans.map(loan => (
                                <div key={loan.id} className="p-3 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-between hover:bg-white transition-all">
                                   <div className="flex-1 pr-2 overflow-hidden">
                                     <p className="font-black text-indigo-950 text-[9px] uppercase truncate">{loan.bookTitle}</p>
                                   </div>
                                   <div className="flex gap-1">
                                     <button onClick={() => handleAction(loan.bookId, 'Pemulangan', student.name, 'Murid')} className="p-1.5 bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-600 hover:text-white transition shadow-sm"><CheckCircle size={14}/></button>
                                     <button onClick={() => handleAction(loan.bookId, 'Pulang Rosak/Hilang', student.name, 'Murid')} className="p-1.5 bg-rose-100 text-rose-700 rounded-lg hover:bg-rose-600 hover:text-white transition shadow-sm"><AlertTriangle size={14}/></button>
                                   </div>
                                </div>
                              ))}
                              {activeLoans.length === 0 && <p className="text-center py-4 text-[9px] font-black text-slate-300 uppercase italic">Tiada pinjaman aktif.</p>}
                           </div>
                         )}
                       </div>
                     );
                   ...
                   {members.filter(m => m.type === 'Murid' && m.year === selectedYear).length === 0 && (
                    <div className="col-span-full py-20 text-center font-black text-slate-300 uppercase italic tracking-widest border-2 border-dashed border-slate-100 rounded-[3rem]">Tiada rekod murid Tahun {selectedYear}.</div>
                   )}
                </div>
              )}
            </div>
          ...
          {activeTab === 'admin' && isAdminAuthenticated && (
            <div className="space-y-8 animate-in fade-in">
              <div className="flex overflow-x-auto gap-2 no-scrollbar bg-white p-2.5 rounded-[2rem] border-2 shadow-sm border-slate-200">
                  ...
                  <button onClick={() => setAdminSubTab('system')} className={`px-8 py-3 text-[10px] font-black rounded-xl transition-all uppercase tracking-widest ${adminSubTab === 'system' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-600 hover:bg-slate-50'}`}>Sync & Akses</button>
              </div>

              {/* TAB GANTI NILAI (ALIRAN TUNAI) */}
                ...
                <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 shadow-sm overflow-hidden animate-in fade-in">
                  <div className="p-10 border-b flex justify-between items-center bg-slate-50">
                     <div className="flex items-center gap-3">
                        ...
                        <h3 className="font-black uppercase tracking-tighter text-indigo-950 text-xl">Rekod Ganti Nilai (Tunai)</h3>
                     </div>
                     <span className="bg-emerald-600 text-white px-6 py-2 rounded-full font-black text-xs">JUMLAH: RM {transactions.filter(t => t.resolutionMethod === 'Tunai').reduce((acc, t) => acc + (t.fineAmount || 0), 0).toFixed(2)}</span>
                  </div>
                  <div className="overflow-x-auto no-scrollbar">
                    <table className="w-full text-left">
                      <thead className="bg-slate-50 text-[10px] uppercase font-black text-slate-500 border-b-2 border-slate-100">
                        <tr><th className="px-10 py-6">Tarikh & Masa</th><th className="px-10 py-6">Peminjam</th><th className="px-10 py-6">Butiran Buku</th><th className="px-10 py-6 text-right">Jumlah (RM)</th></tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 text-indigo-950">
                        {transactions.filter(t => t.resolutionMethod === 'Tunai').map(t => (
                          <tr key={t.id} className="text-xs hover:bg-emerald-50/50 transition duration-300">
                            <td className="px-10 py-6 font-bold text-slate-500">{t.timestamp}</td>
                            <td className="px-10 py-6 font-black uppercase text-indigo-900">{t.userName}</td>
                            <td className="px-10 py-6 font-bold uppercase truncate max-w-[250px]">{t.bookTitle}</td>
                            <td className="px-10 py-6 font-black text-emerald-700 text-right">+{t.fineAmount?.toFixed(2)}</td>
                          </tr>
                        ))}
                        {transactions.filter(t => t.resolutionMethod === 'Tunai').length === 0 && (
                          <tr><td colSpan={4} className="p-20 text-center font-black text-slate-300 uppercase italic tracking-widest">Tiada rekod bayaran ganti nilai tunai dikesan.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
              ...
              {/* SUBTAB ADMIN LAIN */}
              {adminSubTab === 'system' && (
                <div className="max-w-4xl space-y-8 animate-in slide-in-from-bottom-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-white rounded-[3rem] border-2 border-slate-200 p-10 shadow-xl border-b-8 border-indigo-200">
                      <div className="flex items-center gap-4 mb-8 pb-4 border-b">
                        <KeyRound size={28} className="text-indigo-600" />
                        <h3 className="text-xl font-black text-indigo-950 uppercase tracking-tighter">Akses Pentadbir</h3>
                      </div>
                      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); alert("Credentials berjaya dikemaskini."); }}>
                         <div className="relative">
                            <label className="text-[9px] font-black uppercase text-indigo-400 absolute left-6 top-3 tracking-widest">ID Admin Baharu</label>
                            <input type="text" className="w-full px-6 pt-8 pb-4 rounded-3xl border-2 border-slate-100 bg-slate-50 font-black text-indigo-950 uppercase outline-none focus:border-indigo-600 transition" value={adminSettings.adminId} onChange={(e) => setAdminSettings({...adminSettings, adminId: e.target.value})} />
                         </div>
                         <div className="relative">
                            <label className="text-[9px] font-black uppercase text-indigo-400 absolute left-6 top-3 tracking-widest">Kata Laluan Baharu</label>
                            <input type="password" placeholder="••••••••" className="w-full px-6 pt-8 pb-4 rounded-3xl border-2 border-slate-100 bg-slate-50 font-black text-indigo-950 outline-none focus:border-indigo-600 transition" value={adminSettings.adminPass} onChange={(e) => setAdminSettings({...adminSettings, adminPass: e.target.value})} />
                         </div>
                         <button type="submit" className="w-full py-5 bg-indigo-950 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-xl flex items-center justify-center gap-3 hover:bg-black transition-all active:scale-95 border-b-4 border-slate-800"><Save size={18}/> Simpan Akses</button>
                      </form>
                    </div>
              ...
              {adminSubTab === 'damages' && (
                <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 shadow-sm overflow-hidden animate-in fade-in">
                   <div className="p-10 border-b flex justify-between items-center bg-slate-50">
                     <h3 className="font-black uppercase tracking-tighter text-indigo-950 text-xl">Rekod Kerosakan Tertunggak</h3>
                   </div>
                     ...
                     {transactions.filter(t => t.status === 'Rosak/Hilang' && t.resolutionStatus === 'Tertunggak').map(t => (
                        <div key={t.id} className="p-8 flex items-center justify-between hover:bg-slate-50 transition-all group">
                           ...
                           <div className="flex gap-2">
                              <button onClick={() => handleResolveDamage(t.id, 'Buku')} className="px-5 py-3 bg-indigo-600 text-white rounded-xl font-black uppercase text-[9px] shadow-md border-b-2 border-indigo-800 hover:bg-indigo-700 transition">Ganti Buku</button>
                              <button onClick={() => handleResolveDamage(t.id, 'Tunai')} className="px-5 py-3 bg-emerald-600 text-white rounded-xl font-black uppercase text-[9px] shadow-md border-b-2 border-emerald-800 hover:bg-emerald-700 transition">Bayar Tunai</button>
                           </div>
                        </div>
                     ))}
              ...
              {adminSubTab === 'session' && (
                <div className="max-w-xl animate-in slide-in-from-bottom-4">
                   <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 p-10 shadow-xl border-b-8 border-indigo-200">
                      ...
                      <p className="text-sm text-slate-500 font-medium leading-relaxed mb-10">Urus pertukaran sesi tahunan atau bersihkan log sistem untuk sesi baru.</p>
                      <div className="space-y-4">
                         <button onClick={() => { if(confirm("ADAKAH ANDA INGIN MENAIKKAN TAHUN SEMUA MURID? Tahun 6 akan dikeluarkan.")) { setMembers(prev => prev.map(m => m.type === 'Murid' && m.year ? {...m, year: m.year + 1} : m).filter(m => !(m.type === 'Murid' && m.year && m.year > 6))); alert("Naik Sesi Berjaya!"); } }} className="w-full py-5 bg-indigo-600 text-white rounded-3xl font-black uppercase text-[10px] tracking-widest shadow-xl border-b-4 border-indigo-800 hover:bg-indigo-700 transition active:scale-95"><ArrowUpFromLine size={18}/> Naikkan Tahun Murid</button>
                         <button onClick={() => { if(confirm("PENGESAHAN: PADAM SEMUA LOG TRANSAKSI?")) { setTransactions([]); alert("Log dipadam."); } }} className="w-full py-5 bg-white border-2 border-slate-200 text-indigo-950 rounded-3xl font-black uppercase text-[10px] tracking-widest shadow-sm hover:bg-slate-50 transition active:scale-95"><RotateCcw size={18}/> Reset Log Aktiviti</button>
                      </div>
                   </div>
                </div>
              )}
              ...
              {adminSubTab === 'overview' && (
                <div className="space-y-8 animate-in fade-in">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <button onClick={fetchAiInsight} className="bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-xl hover:bg-indigo-700 transition relative overflow-hidden active:scale-95 border-b-4 border-indigo-800 group">
                       <Sparkles size={24} className="mb-2 text-indigo-200 animate-pulse" />
                       <p className="text-[10px] font-black uppercase tracking-widest mb-1 leading-none text-indigo-200 group-hover:text-white">AI Analisa</p>
                       <p className="text-2xl font-black uppercase tracking-tighter leading-none text-white">Insight Pintar</p>
                    </button>
                  </div>
                  {isAiLoading && <div className="p-10 text-center animate-pulse font-black text-indigo-600 uppercase tracking-widest text-sm italic">Menganalisa Data...</div>}
                  {aiInsight && !isAiLoading && (
                    <div className="bg-indigo-50 border-2 border-indigo-100 p-10 rounded-[3rem] animate-in slide-in-from-top-4 shadow-sm border-b-8 border-indigo-200">
                      <div className="flex items-center gap-3 mb-4 text-indigo-700 font-black uppercase text-xs tracking-widest border-b border-indigo-200 pb-4"><Sparkles size={16} /> Analisa Sistem Pintar</div>
              ...
              {adminSubTab === 'members' && (
                <div className="space-y-8 animate-in fade-in">
                  <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                     ...
                     <button onClick={() => { setNewMember({...newMember, type: adminMemberView}); setIsAddingMember(true); }} className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg flex items-center gap-2 hover:bg-indigo-700 transition active:scale-95 border-b-4 border-indigo-800"><UserPlus size={18} /> Daftar Ahli Baru</button>
                  </div>
                  {adminMemberView === 'Guru' ? (
                      ...
                      {members.filter(m => m.type === 'Guru').map(member => (
                        <div key={member.id} onClick={() => { setSelectedMemberDetail(member); setEditedMemberName(member.name); setIsEditingMemberName(false); setIsMemberDetailOpen(true); }} className="bg-white rounded-[2.5rem] border-2 border-slate-200 p-6 shadow-sm hover:shadow-xl hover:border-indigo-500 transition-all group cursor-pointer flex items-center justify-between">
                           <div className="flex items-center gap-4">
                              <div className="w-14 h-14 bg-indigo-50 text-indigo-700 rounded-2xl flex items-center justify-center font-black text-xl border border-indigo-100 group-hover:bg-indigo-600 group-hover:text-white transition-all">{member.name.charAt(0)}</div>
                              <div className="flex-1 min-w-0">
                                <h4 className="font-black text-xs uppercase text-indigo-950 truncate group-hover:text-indigo-600">{member.name}</h4>
                                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-1">{getActiveLoans(member.name).length} Pinjaman</p>
                              </div>
                           </div>
                    ...
                    <div className="space-y-6">
                      {YEARS.map(year => (
                        <div key={year} className="bg-white rounded-[2.5rem] border-2 border-slate-200 shadow-sm overflow-hidden">
                           <button onClick={() => { const s = new Set(expandedYears); expandedYears.has(year) ? s.delete(year) : s.add(year); setExpandedYears(s); }} className={`w-full px-8 py-5 flex items-center justify-between transition-all ${expandedYears.has(year) ? 'bg-indigo-50/50' : 'hover:bg-slate-50'}`}>
                              <h4 className="font-black text-indigo-900 uppercase tracking-tighter flex items-center gap-2">Murid Tahun {year} <span className="text-[9px] bg-indigo-100 px-3 py-1 rounded-full text-indigo-800 font-black">{members.filter(m => m.type === 'Murid' && m.year === year).length} Ahli</span></h4>
                              {expandedYears.has(year) ? <ChevronDown size={20} className="text-indigo-600" /> : <ChevronRight size={20} className="text-slate-300" />}
                           </button>
                           {expandedYears.has(year) && (
                              <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 border-t-2 border-slate-50">
                                {members.filter(m => m.type === 'Murid' && m.year === year).map(student => (
                                  <div key={student.id} onClick={() => { setSelectedMemberDetail(student); setEditedMemberName(student.name); setIsEditingMemberName(false); setIsMemberDetailOpen(true); }} className="bg-slate-50 rounded-2xl p-4 shadow-sm hover:border-indigo-400 border-2 border-transparent transition-all group cursor-pointer flex items-center gap-3">
                                    <div className="w-10 h-10 bg-white text-indigo-600 rounded-xl flex items-center justify-center font-black group-hover:bg-indigo-600 group-hover:text-white transition-all">{student.name.charAt(0)}</div>
                                    <h4 className="font-black text-[10px] uppercase truncate text-indigo-950">{student.name}</h4>
                                  </div>
                                ))}
                              </div>
                           )}
              ...
              {adminSubTab === 'manage' && (
                <div className="space-y-4 animate-in fade-in">
                  <div className="flex justify-between items-center mb-6">
                     <h3 className="text-xl font-black uppercase tracking-tighter text-indigo-950">Pengurusan Buku</h3>
                     <button onClick={() => { setIsAddingBook(true); setNewBook({ title: '', year: 1, type: 'Buku Teks', stock: 0, price: 0 }); }} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-black uppercase text-[10px] shadow-lg hover:bg-indigo-700 transition active:scale-95 border-b-2 border-indigo-800"><BookPlus size={18} className="inline mr-2"/> Daftar Buku Baru</button>
                  </div>
                    ...
                    {books.sort((a,b) => a.year - b.year || a.title.localeCompare(b.title)).map(book => (
                      <div key={book.id} className="bg-white p-6 rounded-3xl border-2 border-slate-200 flex items-center justify-between hover:border-indigo-400 transition-all shadow-sm group">
                         <div className="flex-1 pr-4">
                           <h4 className="font-black text-[11px] uppercase text-indigo-950 leading-tight group-hover:text-indigo-600">{book.title}</h4>
                         ...
                         <div className="flex items-center gap-8">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => { setBookToEdit({...book}); setIsEditingBook(true); }} className="p-3 bg-slate-50 text-slate-500 hover:text-indigo-600 rounded-xl shadow-sm transition-colors hover:bg-indigo-50"><Edit2 size={18}/></button>
                              <button onClick={() => handleRemoveBook(book.id)} className="p-3 bg-slate-50 text-slate-500 hover:text-rose-600 rounded-xl shadow-sm transition-colors hover:bg-rose-50"><Trash2 size={18}/></button>
                            </div>
                         </div>
                      </div>
                    ))}
          ...
          {activeTab === 'profile' && (
            <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-bottom-6">
              <div className="bg-white rounded-[3.5rem] border-2 border-slate-200 p-10 shadow-xl text-center relative overflow-hidden group hover:border-indigo-400 transition-all border-b-8 border-indigo-500">
                ...
                <div className="w-24 h-24 bg-indigo-600 text-white rounded-[2.5rem] flex items-center justify-center text-4xl font-black mx-auto mb-6 shadow-xl relative z-10 border-4 border-white group-hover:rotate-3 transition-transform">{(userName || 'G').charAt(0)}</div>
                ...
                <div className="grid grid-cols-2 gap-4 relative z-10 max-w-lg mx-auto">
                   <div className="p-6 bg-slate-50 border-2 border-slate-100 rounded-3xl group hover:border-indigo-300 transition-all shadow-sm"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-none">Pinjaman Aktif</p><p className="text-2xl font-black text-indigo-600 leading-none">{getActiveLoans(userName).length}</p></div>
                   <div className="p-6 bg-slate-50 border-2 border-slate-100 rounded-3xl group hover:border-emerald-300 transition-all shadow-sm"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-none">Status Akaun</p><p className="text-[10px] font-black text-emerald-600 uppercase leading-none text-center">AKTIF & SYNC</p></div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[3.5rem] border-2 border-slate-200 shadow-xl overflow-hidden">
                <div className="p-8 border-b-2 border-slate-100 bg-slate-50/80 flex items-center gap-3">
                  ...
                  <h4 className="font-black text-indigo-950 uppercase tracking-tighter text-sm">Pinjaman Aktif Anda</h4>
                </div>
                <div className="p-8 space-y-4">
                  {getActiveLoans(userName).map(loan => (
                    <div key={loan.id} className="p-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:border-indigo-400 transition-all shadow-sm">
                       ...
                       <button onClick={() => handleAction(loan.bookId, 'Pemulangan', userName, 'Guru')} className="px-8 py-3 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg hover:bg-emerald-700 transition active:scale-95 border-b-4 border-emerald-800">Pulangkan Buku</button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === 'history' && (
            <div className="bg-white rounded-[3rem] border-2 border-slate-200 shadow-sm overflow-hidden animate-in fade-in">
               <div className="p-10 border-b bg-slate-50 flex items-center justify-between">
                  <h3 className="text-xl font-black text-indigo-900 uppercase tracking-tighter">Rekod Aktiviti Transaksi</h3>
               ...
               <table className="w-full text-left">
                 <thead className="bg-slate-50 text-[10px] uppercase font-black text-slate-500 border-b-2 border-slate-100 tracking-widest">
                   <tr><th className="px-10 py-6">Masa</th><th className="px-10 py-6">Peminjam</th><th className="px-10 py-6">Tindakan</th><th className="px-10 py-6">Judul Buku</th><th className="px-10 py-6">Status</th></tr>
                 </thead>
        ...
        {/* MODAL PENDAFTARAN BUKU BARU (ADMIN) */}
        {(isAddingBook || isEditingBook) && (
          <div className="fixed inset-0 bg-indigo-950/90 backdrop-blur-md z-[200] flex items-center justify-center p-4">
             <div className="bg-white w-full max-w-xl rounded-[3rem] p-10 shadow-2xl animate-in zoom-in border-4 border-indigo-400">
                <div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-black uppercase tracking-tighter text-indigo-950">{isAddingBook ? 'Daftar Buku Baru' : 'Kemaskini Data'}</h3><button onClick={() => {setIsAddingBook(false); setIsEditingBook(false);}} className="text-slate-400 hover:text-rose-500 transition shadow-sm hover:rotate-90"><X size={24}/></button></div>
                <div className="space-y-6">
                   <div className="relative"><label className="text-[9px] font-black uppercase text-indigo-400 absolute left-6 top-3 tracking-widest">Judul Buku</label><input type="text" className="w-full px-6 pt-8 pb-4 rounded-3xl border-2 border-slate-100 bg-slate-50 font-black uppercase text-indigo-950 outline-none focus:border-indigo-600 transition" value={isAddingBook ? newBook.title : bookToEdit?.title} onChange={(e) => isAddingBook ? setNewBook({...newBook, title: e.target.value.toUpperCase()}) : setBookToEdit({...bookToEdit!, title: e.target.value.toUpperCase()})} /></div>
                   <div className="grid grid-cols-2 gap-4">
                      <div className="relative">
                        <label className="text-[9px] font-black uppercase text-indigo-400 absolute left-6 top-3 tracking-widest">Kategori</label>
                      ...
                      <div className="relative">
                        <label className="text-[9px] font-black uppercase text-indigo-400 absolute left-6 top-3 tracking-widest">Tahun</label>
                        <select className="w-full px-6 pt-8 pb-4 rounded-3xl border-2 border-slate-100 bg-slate-50 font-black text-indigo-950 outline-none appearance-none focus:border-indigo-600 transition" value={isAddingBook ? newBook.year : bookToEdit?.year} onChange={(e) => isAddingBook ? setNewBook({...newBook, year: Number(e.target.value)}) : setBookToEdit({...bookToEdit!, year: Number(e.target.value)})}>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                     ...
                     <div className="relative"><label className="text-[9px] font-black uppercase text-indigo-400 absolute left-6 top-3 tracking-widest">Stok Semasa</label><input type="number" className="w-full px-6 pt-8 pb-4 rounded-3xl border-2 border-slate-100 bg-slate-50 font-black text-indigo-950 outline-none focus:border-indigo-600 transition" value={isAddingBook ? newBook.stock : bookToEdit?.stock} onChange={(e) => isAddingBook ? setNewBook({...newBook, stock: Number(e.target.value)}) : setBookToEdit({...bookToEdit!, stock: Number(e.target.value)})} /></div>
                   </div>
                   <button onClick={isAddingBook ? handleAddNewBook : handleUpdateBook} className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-xl hover:bg-indigo-700 transition active:scale-95 border-b-4 border-indigo-800">Sahkan Maklumat Buku</button>
                </div>
             </div>
          </div>
        )}

        {/* MODAL DAFTAR AHLI BARU (ADMIN) */}
        {isAddingMember && (
          <div className="fixed inset-0 bg-indigo-950/90 backdrop-blur-md z-[200] flex items-center justify-center p-4">
             <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl animate-in zoom-in border-4 border-indigo-400">
                <div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-black uppercase tracking-tighter text-indigo-950 leading-none">Daftar {newMember.type}</h3><button onClick={() => setIsAddingMember(false)} className="text-slate-400 hover:text-rose-500 transition shadow-sm"><X size={24}/></button></div>
                <div className="space-y-6">
                   <div className="relative"><label className="text-[9px] font-black uppercase text-indigo-400 absolute left-6 top-3 tracking-widest">Nama Penuh</label><input type="text" placeholder="HURUF BESAR" className="w-full px-6 pt-8 pb-4 rounded-3xl border-2 border-slate-100 bg-slate-50 font-black uppercase text-indigo-950 outline-none focus:border-indigo-600 transition" value={newMember.name} onChange={(e) => setNewMember({...newMember, name: e.target.value.toUpperCase()})} /></div>
                   ...
                   <button onClick={handleAddMember} className="w-full py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-xl hover:bg-indigo-700 transition active:scale-95 border-b-4 border-indigo-800">Daftar Ahli Baru</button>
                </div>
             </div>
          </div>
        )}

        {/* Modal Pinjaman Pukal (Murid) */}
        ...
        {/* Modal Detail Member (Quick Actions) */}
        {isMemberDetailOpen && selectedMemberDetail && (
          <div className="fixed inset-0 bg-indigo-950/90 backdrop-blur-md z-[200] flex items-center justify-center p-4">
             <div className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in border-4 border-indigo-400">
                <div className="p-10 border-b-2 border-indigo-50 bg-indigo-50/50 flex justify-between items-center">
                   <div className="flex items-center gap-4">
                      <div className="w-16 h-16 bg-indigo-600 text-white rounded-[1.5rem] flex items-center justify-center font-black text-2xl shadow-lg border-2 border-white transition-transform group-hover:rotate-3">{selectedMemberDetail.name.charAt(0)}</div>
                      <div className="flex-1">
                        {!isEditingMemberName ? (
                          <div className="flex items-center gap-2">
                ...
                <div className="p-10 overflow-y-auto no-scrollbar flex-1 space-y-8 bg-white">
                   <div className="flex justify-between items-center border-b-2 border-indigo-50 pb-3">
                      <h4 className="text-[10px] font-black uppercase text-indigo-900 tracking-widest flex items-center gap-2"><BookOpen size={14} className="text-indigo-600"/> Pinjaman Aktif Semasa</h4>
                      <button onClick={() => { setBorrowingStudent(selectedMemberDetail); setIsBorrowModalOpen(true); }} className="px-4 py-2 bg-indigo-600 text-white rounded-xl font-black uppercase text-[9px] shadow-md flex items-center gap-1 hover:bg-indigo-700 transition active:scale-95 border-b-2 border-indigo-800"><Plus size={14}/> Pinjaman Baru</button>
                   </div>
                   <div className="space-y-3">
                      ...
                      )) : <p className="text-center py-10 text-[10px] font-black text-slate-300 uppercase italic tracking-widest">Tiada rekod pinjaman ditemukan.</p>}
                   </div>
                </div>
                <div className="p-8 bg-slate-50 border-t-2 border-slate-100 flex justify-between items-center">
                   <p className="text-[8px] font-black text-slate-400 uppercase italic">ID Sistem: {selectedMemberDetail.id}</p>
                   <button onClick={() => handleRemoveMember(selectedMemberDetail.id)} className="px-6 py-3 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl font-black uppercase text-[10px] flex items-center gap-2 hover:bg-rose-600 hover:text-white transition-all shadow-sm active:scale-95"><Trash2 size={16}/> Padam Profil</button>
                </div>
             </div>
          </div>
        )}
